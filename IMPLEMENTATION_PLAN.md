...
|---|---------|--------|-------------|
| 5.1 | **Notifly: นักเรียนไม่ต่อคอร์ส** — ≥85% ตัวแดง, ≥80% ตัวเหลือง, ≥70% ตัวเขียว | ✅ Done | Runtime alerts บน `admin/notifications` + quick send reminder |
| 5.2 | **Notifly: คลาสคนน้อย** — 1 คน สีแดง, 2 คน สีเหลือง, >2 คน สีเขียว | ✅ Done | Runtime alerts บน `admin/notifications` |
| 5.3 | **แจ้งเตือนลูกค้าเดือนก่อนไม่ลง** — เตือนให้กลับมาเรียน | ✅ Done | มี follow-up alerts + quick send reminder |
| 5.4 | **แจ้งเตือนลูกค้ากลับมาเรียน** — ติดตามลูกค้าเก่า | ✅ Done | มี follow-up alerts สำหรับลูกค้าที่หายไปเกิน 1 เดือน |
| 5.5 | **Notification events ครบทุก event** — จองใหม่, แนบสลิป, ยืนยันชำระ, เปลี่ยนวัน, ร้องเรียน, โปรแกรมสอนรอตรวจ | ✅ Done | Hook แล้วใน user/admin/coach flows หลัก |
...
|---|-------|--------|-----------|
| 6.1 | **Level categories ไม่ตรง Requirement** | ✅ Done | ปรับหน้า `dashboard/progress`, `coach/levels`, `coach/students` ให้ใช้ range เดียวกัน |
| 6.2 | **โค้ชดูชั่วโมงเรียนของนักเรียน** | ✅ Done | หน้า `coach/students` แสดงชั่วโมงเรียนสะสมจาก `total_sessions * duration_hours` |
| 6.3 | **แถบสถานะตอนเลือกจำนวนครั้ง** | ✅ Done | แสดง status badge/warning แล้วทั้ง `dashboard/booking` และ `admin/booking` |
