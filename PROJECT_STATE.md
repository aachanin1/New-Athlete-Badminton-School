...
## Current State — Task10 re-wallet correction verified locally; apply and Resume pending

**DEVELOPING.** Owner authorized the narrow database correction and conditional Resume on 2026-09-24. The tested local migration is ready for normal publication. Production is still **Paused / revision 2 / three controls OFF / cron inactive** at the latest read; apply and authenticated post-migration checks must precede Resume. The earlier 28-bill cancellation is retained and must not be repeated.

### Current Project Matrix

| Field | Current value |
| --- | --- |
| Active Task / Actor / Authorization | Task10 re-wallet state-reader correction, one migration, protected regressions, apply while Paused, read-only Production verification, conditional Resume and natural worker checks. Developer Codex operates under Owner authority; PM remains read-only. |
| Task Status / Task Done / READY | DEVELOPING / No / No. Local verification passed; required Production gates remain pending. |
| Scope / Changed Paths | Database function 1 / migration 1 / tests 2 / Docs4 = 7 paths. Application UI0, test-helper0, config0, dependencies0. One disclosed technical test-path addition under the AGENTS dependency rule; no new business behavior or scope breach. |
| Blast Radius / Protected Domains | Kids family state read and the existing consume caller; shared Admin Makeup page. Duplicate guard moves after candidate eligibility. Canonical structure, child/root identity, spent-source exclusion, transition evidence, quota/N/D/M/expiry, permissions and transaction locks remain unchanged. Resume also re-enables existing pricing and expiry behavior. |
| Branch / Upstream | spike/next-major-security-upgrade / origin/spike/next-major-security-upgrade. |
| Local HEAD / Remote HEAD / Ahead-Behind | Fresh Gate0 f7223caf799b8c438bbb0466afed74c70c84c0cd matched live remote, 0/0. The containing corrective commit is separate from the accepted web SHA; exact post-publication SHA/live equality is recorded in rewallet-corrective-20260924T133200Z/source-publication.json. |
| Index / Worktree / Committed / Pushed | Only the seven authorized paths form this corrective publication. Complete staged diff, scope/privacy and documentation checks precede normal commit/push. The external publication record owns final read-back evidence; remote apply is gated on successful publication. |
| Source Complete / Current Source / Pushed Source | Local database correction complete. Accepted web source remains 1fba7bab987e8455824889563604063813e08855. Unchanged application/config/dependency bytes and original migrations verified; no replacement web build. Corrective migration/test source is this separate publication. |
| Tests Passed / Fresh | Full Task10 84/84 (79 existing plus 5 new), Wallet focused45/45, TypeScript, src lint and explicit modified-test lint passed. New checks prove both UUID orders, Paused/Active reads, one source/use, actual local consume/replay/concurrent duplicate request, changed-payload rejection, no revival, true duplicate rejection, and loaded migration/security metadata. Existing full-suite checks cover quota/D/M, Wallet-versus-Makeup races, pricing/old bills, receipt/expiry and Family atomicity. Counts are not added together. |
| Retained Evidence | September22 Task1079/Booking23/Makeup5/Wallet45/Admin39/retrospective38 and accepted Production build remain historical evidence. Fresh84 and freshWallet45 above supersede their respective old executions for this correction; no Production business UAT or rebuild is claimed. |
| Failed Evidence / Root Cause | First full run83/84: an existing real-time UI fixture used seconds outside the HH:mm template API contract and failed its day click. The bounded correction aligns only the test target to a real whole minute30–90seconds ahead; all assertions/timeouts remain unchanged. Fresh rerun84/84 passed. Original-function regression reproduced TASK10_AMBIGUOUS_SOURCE after fixing one synthetic Paused fixture flag mismatch. Valid re-wallet descendant sorted before the spent original caused premature duplicate-root rejection. The old function reproduced the same error for both affected Production families in READ ONLY transactions. Initial fixture failure and red regression are retained separately. |
| Migration Source / Applied | 20260924133852_task10_rewallet_source_eligibility.sql, SHA256 a9904c31e96e27e8fd40b61f803da271ecea0f87ca01b2ddf52293a5c4c92990. Source M1–M5 unchanged; new migration applied in the verified disposable only. Production apply pending. No other function, grants or customer-data DML in the migration. |
| Deployed Source / Deployment ID / Production Routing | Web SHA1fba7bab987e8455824889563604063813e08855 / dpl_5bJmN6piCPmThycQEBSr9Rvqj2MF. Fresh Vercel project/team, READY production target, four customer aliases and health verified. Production Git branch main; this spike branch produces Preview only. No rebuild/redeploy/Promotion/routing operation. |
| Feature Enabled / Activation / Scheduler | Fresh paused/revision2; pricing/makeup/expiry false; original effective_at 2026-09-24T12:45:14.241075Z retained. One every-minute limit50 cron inactive; four prior natural runs, no new manual worker. Conditional Resume is authorized, not executed at this publication. |
| Allowlisted / Environment / Permissions | Existing bindings and permissions unchanged. Real authorized school SuperAdmin identity and session verified; function owner/security/search_path/ACL preserved locally and baseline recorded remotely. No allowlist/environment/secret/Auth/RLS change. |
| Production Active / Customer Impact | Accepted web is customer Production. New Kids booking and family Makeup remain blocked by existing pause guards; expiry is stopped. The old reader still needs the approved migration. Do not announce ready from local test results. |
| Production UAT / Controlled Write UAT / Owner Acceptance | Fresh Developer-controlled writes only in new disposable Task10Rewallet20260924 API60321/DB60322. Production browser checks are read-only. Prior PM-operated QA/Owner acceptance remains historical PASS for its tested scope, not acceptance of this migration or Production business UAT. |
| Production Data Changed / Data Repaired | No business write or repair in this corrective round so far. Original cutover28bills/104sessions stay cancelled. Actual baseline protected exact rows match; no refund, revival, uncancellation or replay. Migration/control/event effects are still pending their gates. |
| Protected Data / Live Traffic | Fresh18 reconciliation checks retained exact cutover/Wallet/financial evidence. Three Head Coach attendance additions at13:01UTC preceded this round; removing only those additions reproduces the prior attendance hash. An Adult pending bill created13:54UTC is outside the cutover set, not due at13:56UTC, deadline26September11:00UTC. Refresh due rows before Resume; do not claim all customer traffic is frozen. |
| Financial Impact | No Developer customer booking/payment/Wallet/Makeup transaction on Production. Existing50,846THB cancelled bill value is retained history, not a new cancellation, refund or automatic revenue-loss calculation. Payments, allocations, coupons, expenses and payroll baselines are captured for postchecks. |
| Local Preservation | New isolated target physical container/volume/workdir/network and DB/Auth/REST/Storage bindings verified before harness resets. Original54321/55321/56321/57321/58321 and cutover59321 preserved. No Owner fixture reset/reseed/replay, cleanup or stop. Auth/browser/Node use real time; only existing local business-clock mechanism is used. |
| Unknowns / Limits | Historical stream-close/ERR_ABORTED, credential validity and source exposure remain Unknown. Seven pre-existing local advisor warnings are outside scope; none identifies the replaced function. Local reset-time Auth reconnect messages recovered through existing readiness checks, without changing timeouts/assertions. No security certification. |
| Documentation Drift | Earlier “awaiting correction authorization” is superseded by the latest Owner approval. Earlier cutover/pause and PM acceptance records are preserved as history. Current apply/Resume status is explicitly pending. |
| Blocker / Remaining Work / Next Gate | No unresolved local failure. Publish this exact tested migration, fresh-check Paused/deployed baseline, apply only it, verify Production school-role Makeup/pricing/payment and protected data, then Resume with fresh revision and observe at least two natural worker runs. Pause immediately on attributable regression. |
| Parking Lot | No new authorization; do not start other work automatically. |

Private evidence: C:/Users/aacha/AppData/Local/Temp/task10-calendar-uat-20260922/rewallet-corrective-20260924T133200Z. Exact customer rows, accounts and runtime logs stay outside Git/deployment.
[Corrective decision](AGENTS.md#task10-rewallet-corrective-decision-2026-09-24); [dated local verification](DEVELOPMENT_TODO.md#task10-rewallet-local-verified-2026-09-24). Old cutover evidence remains in owner-cutover-20260924T115900Z.
