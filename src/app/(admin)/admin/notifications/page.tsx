...
async function fetchRecommendationWorkspace({
  ...
  const bookings = bookingRows.flatMap((row): RecommendationBookingInput[] => {
    ...
    return [{
      ...
      learnerName: row.child_id ? childNameById.get(row.child_id) || 'ต้องตรวจสอบรายชื่อผู้เรียน' : ownerName,
  ...
  const sessions: RecommendationSessionInput[] = sessionRows.map((row) => ({
    ...
    branchName: row.branches?.name || '-',
    childId: row.child_id,
    learnerName: row.child_id ? childNameById.get(row.child_id) || 'ต้องตรวจสอบรายชื่อผู้เรียน' : null,
    status: row.status,
